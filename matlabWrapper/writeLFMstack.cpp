...
*       writeLFMstack(im, filename, numThreads, pixelSize, blockSize, compressionType, metadata, predictor, Nnum, imageOrVideo)  
*/
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
    ...
    switch( nrhs )
    {
        ...
        case 8://metadata is added
			...
			if( mxIsEmpty(prhs[7]) == false )
			{
				imgIO.header.headerVersion = (int)(mxGetPr(prhs[7])[0]);
			}
            break;
        case 9://metadata is added
            if (mxIsEmpty(prhs[2]) == false)
			{
				numThreads = (int)(mxGetPr(prhs[2])[0]);
			}
	    	...
	    	if( mxIsEmpty(prhs[7]) == false )
            {
              	imgIO.header.headerVersion = (imgIO.header.headerVersion | (uint8_t)(mxGetPr(prhs[7])[0]));
            }
	    	if( mxIsEmpty(prhs[8]) == false )
            {
              	imgIO.header.Nnum = (int)(mxGetPr(prhs[8])[0]);
            }			
            break;
        case 10://metadata is added
            if (mxIsEmpty(prhs[2]) == false)
			{
				numThreads = (int)(mxGetPr(prhs[2])[0]);
			}
			...
			if( mxIsEmpty(prhs[9]) == false )
            {
              	imgIO.header.headerVersion = (imgIO.header.headerVersion | (uint8_t)(mxGetPr(prhs[9])[0]) << 7);
            }						
            break;			
        default:
