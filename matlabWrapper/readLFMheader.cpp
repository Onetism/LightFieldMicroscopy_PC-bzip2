...
*     
*     \brief Set of utilities to read/write LFM format
*       header = readLFMheader(filename)     
*/
...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs,const mxArray *prhs[])
{
	...
	const int nfields = 8;//it has to match fieldnames
	const char *fieldnames[] = { "xyzct", "pixelSize", "dataType", "compressionType", "blockSize", "metadata", "headerVersion", "Nnum"};       /* pointers to field names */
    ...
    //set Nnum
	mxArray *field_value_7 = mxCreateDoubleMatrix(1, 1, mxREAL);
	ptr = mxGetPr(field_value_7);
	ptr[0] = imgIO.header.Nnum;
	mxSetFieldByNumber(plhs[0], 0, 7, field_value_7);//field number is 0-indexed
	//release memory	
    mxFree(filename);
};