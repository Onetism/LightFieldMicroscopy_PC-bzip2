...
public:
	...
	//header fields
	std::uint8_t					headerVersion;		//0 bit: 1:image, 0:video, 1:7bit: predictor   
	std::uint8_t                    Nnum;				//microlens pitch size
	...
	size_t getSizeInBytes()  const{ return KLB_DATA_DIMS * (2 * sizeof(std::uint32_t) + sizeof(float32_t)) + 3 * sizeof(std::uint8_t) + Nb * sizeof(std::uint64_t) + sizeof(char)* (KLB_METADATA_SIZE + 1); };
	size_t getSizeInBytesFixPortion()  const{ return KLB_DATA_DIMS * (2 * sizeof(std::uint32_t) + sizeof(float32_t)) + 3 * sizeof(std::uint8_t) + sizeof(char)* (KLB_METADATA_SIZE + 1); };
	...
	void setHeader(const std::uint32_t xyzct_[KLB_DATA_DIMS], const  KLB_DATA_TYPE dataType_, const  float32_t pixelSize_[KLB_DATA_DIMS] = NULL, const std::uint32_t blockSize_[KLB_DATA_DIMS] = NULL, 
					const KLB_COMPRESSION_TYPE compressionType_ = KLB_COMPRESSION_TYPE::BZIP2, const  char metadata_[KLB_METADATA_SIZE] = NULL, const  std::uint8_t headerVersion_ = KLB_DEFAULT_HEADER_VERSION, const std::uint8_t Nnum_ = 13);
