...
public:
	...
	*/
	...
	float bwt_entropy_2D(uint16_t* In, float *entropy, int is_src);

	int bwt_entropy_comparsion_2D(uint16_t* dpPredict, const uint16_t* dpSrc);

	int huffman_bits_comparsion(uint16_t* dpPredict, const uint16_t* dpSrc);

	int bwt_entropy_comparsion_1D(uint16_t* dpPredict, const uint16_t* dpSrc);

	int image_realign(uint16_t* In, uint16_t* Out);

	int predict_and_2DEntropy(uint16_t *In, uint16_t **out, float *entropy, std::atomic<uint64_t> *blockId, int numPredictors);
...
private:
	...
	int Predictor_both(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int Predictor_angle(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int Predictor_space(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int Predictor_space_GPU(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int Predictor_angle_GPU(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int Predictor_both_GPU(const uint16_t* in, uint16_t* out, int tileSize, int predictors);
	int unPredictor(const int16_t* in, uint16_t* out, int tileSize);
	int unPredictor_space(const int16_t* in, uint16_t* out, int tileSize);
	int unPredictor_angle(const int16_t* in, uint16_t* out, int tileSize);
