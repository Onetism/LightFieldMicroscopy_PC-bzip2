...
//#define __DEBUG__
//  unsigned int *global_int_original_string = NULL; 
//  unsigned int *global_first_sort_rank = NULL;
...
class Bar
{
	...
	public:
		__host__
		Bar(unsigned int *_functor_string,unsigned int *_functor_first_sort_rank,int _currentOffset, int _currentLength, int _totalLength):functor_string(_functor_string), functor_first_sort_rank(_functor_first_sort_rank), currentOffset(_currentOffset), currentLength(_currentLength), totalLength(_totalLength) { }
...
int gpuBlockSort(UChar *block, UInt32 *order, UInt32 *orderFirstSort, UInt32 *orderSecondSort, UInt32 *orderFirstSortRank, Int32 blockSize, Int32* sortingDepth) { 
	...
	if(length!=0) { 
		...
		for(offset = limit; offset < originalLength; offset+=(length/2)) {  
			...
			thrust::sort(
					...
					Bar(global_int_original_string,global_first_sort_rank,offset, length, originalLength)
				    );
	...
	// d_original_string_in = NULL;
	// global_int_original_string = NULL;
	// global_first_sort_rank = NULL;
