...
/*---------------------------------------------------*/
void cuda_BZ2_bsInitWrite ( EState* s )
{
   s->bsLive = 0;
   s->bsBuff = 0;
}
...
static
void cpu_sendMTFValues ( EState* s )
{
...
#           undef BZ_ITUR
      ...
      for (t = 0; t < nGroups; t++)
         cuda_BZ2_hbMakeCodeLengths ( &(s->len[t][0]), &(s->rfreq[t][0]), 
                                 alphaSize, 17 /*20*/ );
   }
   ...
   for (t = 0; t < nGroups; t++) {
      ...
      cuda_BZ2_hbAssignCodes ( &(s->code[t][0]), &(s->len[t][0]), 
...
bool merge_two_sort_arrays ( EState *s ) 
{ 
...
#ifdef PRINT_DEBUG
	printf("cuda_BZ2_compressBlock %d\n",s->blockNo); 
#endif

	// start with a fresh buffer with every block
	cuda_BZ2_bsInitWrite( s ); 
...
Bool cuda_BZ2_compressBlock_only_CPU ( EState* s, Bool is_last_block ) {
...
/*---------------------------------------------------*/
 bool cuda_BZ2_compressBlocks ( cuda_bz_stream* strm)
{
   // num++;
	// gpuSetDevice(num%4);
	gpuSetDevice(0);
   ...
   // for (int i = 0; i < 3; i++)
	...
	/*		struct timespec thread2_T1, thread2_T2;
		...
		/*	struct timespec thread2_work1, thread2_work2;*/
			while(true) {
				if(done_block_sort > done_huff_mtf) {
					...
					// printf("threadID == 1!\n");
					bool ret = merge_two_sort_arrays ( s );
               // if (ret == false)
               //    return false;
					// printf("threadID == 1!\n");
		...
		// 		}
		// 		s = (EState *)strm->state[count];
		// 		if(!cuda_BZ2_compressBlock_only_CPU( s, (Bool) count == strm->state_fill_count)) { 
		// 			printf("cuda_BZ2_compressBlock_only_CPU failed at blockNo %d\n",count);
		// 			exit(2);
		// 		}
		// 	}
		// } 
   ...
   tests = (EState *)strm->state[0];
   return true;
}
/*---------------------------------------------------*/
static
void cpu_generateMTFValues ( EState* s )
{
   UChar   yy[256];
   Int32   i, j;
   Int32   zPend;
   ...
   */
   ...
   for (i = 0; i < s->nblock; i++) {
      UChar ll_i;
      AssertD ( wr <= i, "cpu_generateMTFValues(1)" );
      j = ptr[i]-1; if (j < 0) j += s->nblock;
      ll_i = s->unseqToSeq[block[j]];
      AssertD ( ll_i < s->nInUse, "cpus_generateMTFValues(2a)" );
...
// /*---------------------------------------------------*/
// void cpu_cuda_BZ2_compressBlock ( EState* s, Bool is_last_block )
// {
//    if (s->nblock > 0) {
...
void cuda_BZ2_compressBlocks_pthreads ( cuda_bz_stream* strm)
{

	printf("[cuda_BZ2_compressBlocks_pthreads] Total Blocks : %d, Block Size %d\n", strm->state_fill_count, ((EState*)strm->state[0])->nblockMAX);
	gpuSetDevice(0);
	global_strm = strm;
...
void cuda_BZ2_compressBlocks_without_overlap (cuda_bz_stream* strm)  { 
