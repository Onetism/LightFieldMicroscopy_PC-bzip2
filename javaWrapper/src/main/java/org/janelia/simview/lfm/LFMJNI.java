...
public class LFMJNI< T extends RealType< T > & NativeType< T > > extends LFM< T >
{
    ...
    static {
        ...
        try {
            loader.unpackAndLoadFromResources( "lfm-jni" );
        } catch ( UnsatisfiedLinkError e1 ) {
            try {
                // If loading lfm fails with an UnsatisfiedLinkError,
                ...
                throw new UnsatisfiedLinkError( "[LFM] Failed to unpack or load native LFM libraries.\n" + e2.getMessage() );
            }
        } catch ( Throwable e3 ) {
            throw new UnsatisfiedLinkError( "[LFM] Failed to unpack or load native LFM libraries.\n" + e3.getMessage() );
        }
    ...
    /**
     * Constructor is protected to prevent direct instantiation. Use org.janelia.simview.lfm.LFM.newInstance() instead.
    ...
    public Header readHeader( final String filePath ) throws IOException
    {
        ...
        if ( err != 0 )
            throw new IOException( String.format( "Could not read LFM header of file %s, error code %d.", filePath, err ) );
            ...
            throws IOException
    {
        ...
        if ( err != 0 )
            throw new IOException( String.format( "Could not read LFM file %s, error code %d.", filePath, err ) );
    }
    ...
    // Override default implementations in org.janelia.simview.lfm.LFM to use JNI functions.
            ...
            throws IOException
    {
        ...
        if ( err != 0 )
            throw new IOException( String.format( "Could not read ROI from LFM file %s, error code %d.", filePath, err ) );
    }
