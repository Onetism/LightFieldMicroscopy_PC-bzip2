...
public abstract class LFM< T extends RealType< T > & NativeType< T > >
{
    ...
    public class Header
    {
        ...
        /**
         * Size of LFM blocks (units that are de-/compressed in parallel), in order xyzct
         */
    ...
    /**
     * Returns an instance of a concrete LFM implementation. Such instances should in turn have protected constructors
    ...
    /**
     * Read header from LFM file
     *
     * @param filePath file system path to LFM file
     * @return org.janelia.simview.LFM.Header instance
     * @throws IOException
     */
    ...
    // implementations of LFM should override these functions to read the corresponding data types directly.
            ...
            throws IOException
    {
        ...
        if ( type instanceof GenericByteType ) {
            ...
            throw new IOException( "Unknown or unsupported LFM data type" );
        }
    }
            ...
            throws IOException
    {
        ...
        final long[] lfmMin = new long[ 5 ];
        final long[] lfmMax = new long[ 5 ];
        ...
        while ( cursor.hasNext() ) {
            ...
            cell.min( cellOffset );
            int i = 0;
            for ( int d = 0; d < lfmMin.length; ++d ) {
                if ( header.imageSize[ d ] == 1 ) {
                    lfmMin[ d ] = lfmMax[ d ] = 0;
                } else {
                    lfmMin[ d ] = cellOffset[ i ];
                    lfmMax[ d ] = lfmMin[ d ] + cellDims[ i++ ] - 1;
                }
            }
            switch ( type.getBitsPerPixel() ) {
                case 8:
                    readROIinPlace( filePath, lfmMin, lfmMax, (( ByteArray ) cell.getData()).getCurrentStorageArray() );
                    break;
                case 16:
                    readROIinPlace( filePath, lfmMin, lfmMax, (( ShortArray ) cell.getData()).getCurrentStorageArray() );
                    break;
                case 32:
                    if ( type instanceof IntegerType )
                        readROIinPlace( filePath, lfmMin, lfmMax, (( IntArray ) cell.getData()).getCurrentStorageArray() );
                    else
                        readROIinPlace( filePath, lfmMin, lfmMax, (( FloatArray ) cell.getData()).getCurrentStorageArray() );
                    break;
                case 64:
                    if ( type instanceof IntegerType )
                        readROIinPlace( filePath, lfmMin, lfmMax, (( LongArray ) cell.getData()).getCurrentStorageArray() );
                    else
                        readROIinPlace( filePath, lfmMin, lfmMax, (( DoubleArray ) cell.getData()).getCurrentStorageArray() );
                    break;
                default:
                    throw new IOException( String.format( "Unknown or unsupported LFM data type of file %s.", filePath ) );
            }
        }
    }
        ...
        while ( cursor.hasNext() ) {
            ...
            for ( int d = 0; d < lfmMin.length; ++d ) {
                if ( header.imageSize[ d ] == 1 ) {
                    ...
                    lfmMin[ d ] = xyzctMin[ d ] + cellOffset[ i ];
                    lfmMax[ d ] = lfmMin[ d ] + cellDims[ i++ ] - 1;
                }
            }
            switch ( type.getBitsPerPixel() ) {
     ...
     * LFM block for max performance. Otherwise, all LFM blocks that intersect with multiple ImgCells will be
            ...
            throws IOException
    {
        ...
        while ( cellCur.hasNext() ) {
            ...
            final String cellFilePath = filePath.replace( ".lfm", String.format( ".%s-%s.lfm", minStr, maxStr ) );
    ...
    private ImgPlus< T > imgToImgPlus( final Img< T > img, final Header header, final String name )
    {
        ...
        for ( int d = 0; d < 5; ++d ) {
            if ( header.imageSize[ d ] > 1 ) {
                axisTypes[ i ] = LFMScifio.AXIS_TYPES[ d ];
     ...
     * is stored in as few as possible contiguous arrays and aligned with the LFM blocks to avoid LFM blocks that
    ...
    /**
     * When reading a LFM image as a CellImg, the shape of the ImgCells should be chosen such that the image is stored
     * in as few as possible contiguous arrays. The ImgCells should also be aligned with the LFM blocks to avoid LFM
