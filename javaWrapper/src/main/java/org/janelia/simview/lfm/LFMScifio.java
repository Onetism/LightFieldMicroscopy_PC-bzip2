...
@Plugin( type = Format.class, priority = Priority.FIRST, name = "light field File Format" )
public class LFMScifio extends AbstractFormat
{
    ...
    {
        return new String[]{ "lfm" };
    }

    @Override
    public String getFormatName()
    {
        return "light field File Format";
    }
    ...
    public static class Parser extends AbstractParser< Metadata >
    {
                ...
                throws IOException, FormatException
        {
            ...
            if ( clazz == UnsignedByteType.class )
                ...
                throw new FormatException( String.format( "Unknown or unsupported LFM data type of file %s.", getSource().getFileName() ) );

            iMeta.populate( source.getFileName(), axes, dimensions, format, true, true, false, false, true );
            iMeta.setPlanarAxisCount( 3 ); // should be Klb.LFM_DATA_DIMS (=5), but ImageJ doesn't like that currently
    ...
    public static class Reader extends ByteArrayReader< Metadata >
    {
                ...
                throws FormatException, IOException
        {
            ...
            if ( iMeta.getPlanarAxisCount() == 2 ) {
                ...
                final long[] blockDims = lfm.readHeader( getCurrentFile() ).blockSize;
                ...
                if ( planeIndex < cacheMin || planeIndex > cacheMax ) {
                    ...
                    min[ 2 ] = cacheMin;
                    max[ 2 ] = cacheMax;
                    lfm.readROIinPlace( getCurrentFile(), min, max, cachedPlanes );
                }
            ...
            lfm.readROIinPlace( getCurrentFile(), min, max, plane.getBytes() );
            return plane;
        }
    ...
    public static class Writer< T extends RealType< ? > & NativeType< ? > > extends AbstractWriter< Metadata >
    {
                ...
                throws FormatException, IOException
        {
            ...
            bounds.min( min );
            bounds.max( max );
            log().debug( String.format( "LFM: %s.writePlane(imageIndex %d, planeIndex %d, Plane, min %s, max %s)", getClass().getSimpleName(), imageIndex, planeIndex, Util.printCoordinates( min ), Util.printCoordinates( max ) ) );
            final ImageMetadata iMeta = getMetadata().get( imageIndex );
            ...
            log().debug( "LFM: Dimensions: " + Util.printCoordinates( dimensions ) );
            log().debug( "LFM: Sampling: " + Util.printCoordinates( sampling ) );

            // LFM writes the full image at once, SCIFIO currently handles 2GB max at once.
            long nElements = 1;
            for ( final long d : dimensions )
                nElements *= d;
            nElements *= iMeta.getBitsPerPixel() / 8;
            if ( nElements > Integer.MAX_VALUE )
                throw new FormatException( "Writing LFM files bigger than 2 GB (uncompressed) is currently not supported through SCIFIO/ImageJ." );
            ...
            lfm.writeFull( plane.getBytes(), getMetadata().getDatasetName(), dimensions, type, sampling, null, null, null );
        }
    ...
    public static class Translator extends AbstractTranslator< io.scif.Metadata, Metadata >
    {
        ...
        // after adding the @Plugin annotation, Klb.LFM_DATA_DIMS is unavailable, hence use magic 5;
        private final long[] dimensions = new long[ 5 ];
        ...
        protected void translateImageMetadata( final List< ImageMetadata > src, final Metadata dst )
        {
            ...
            for ( int imageIndex = 0; imageIndex < src.size(); ++imageIndex ) {
                ...
                dstIMeta.setPlanarAxisCount( 3 ); // should be Klb.LFM_DATA_DIMS (=5), but ImageJ doesn't like that currently
            }
        }
    }
}
